...
public class ProfileShipperActivity extends  BaseBottomShipperActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getLayoutInflater().inflate(R.layout.activity_profile_shipper, findViewById(R.id.ship_content_frame));
    ...
    private void saveUserData() {
        ...
        final ProgressDialog mDialog = new ProgressDialog(ProfileShipperActivity.this);

        Account user = CurrentUser.getCurrentUser();

        if (inputUsername.isEmpty() && inputName.isEmpty() && inputPhone.isEmpty() && inputOldPassword.isEmpty() && inputNewPassword.isEmpty()) {
            mDialog.dismiss();
            Toast.makeText(ProfileShipperActivity.this, "Vui lòng không để trống cả 4 chỗ", Toast.LENGTH_SHORT).show();
        }else if(inputName.isEmpty() && inputPhone.isEmpty() && inputOldPassword.isEmpty() && inputNewPassword.isEmpty()) {
            if(user.getUsername().equals(inputUsername)){
                mDialog.dismiss();
                Toast.makeText(ProfileShipperActivity.this, "Tên người dùng đã tồn tại. Vui lòng chọn một cái khác.", Toast.LENGTH_SHORT).show();
            }
            user.setUsername(inputUsername);
        }else if(inputUsername.isEmpty() && inputName.isEmpty() && inputPhone.isEmpty() ){
            if(!user.getPassword().equals(inputOldPassword)){
                mDialog.dismiss();
                Toast.makeText(ProfileShipperActivity.this, "Mật khẩu không khớp với mật khẩu cũ", Toast.LENGTH_SHORT).show();
            }else if(inputNewPassword.length() < 6){
                mDialog.dismiss();
                Toast.makeText(ProfileShipperActivity.this, "Mật khẩu phải có ít nhất 6 ký tự", Toast.LENGTH_SHORT).show();
            }else {
